...
def deviations(d):
    ...
    #positive
    angerDP = abs(ANT_P_POSITIVITY_PERCENTAGE - d['positivePer'])
    angerDNP = abs(ANT_NP_POSITIVITY_PERCENTAGE - d['positivePer'])
    if(angerDP>angerDNP):
    ...
    #negative
    antDP = abs(ANT_P_NEGATIVITY_PERCENTAGE - d['negativePer'])
    antDNP = abs(ANT_NP_NEGATIVITY_PERCENTAGE - d['negativePer'])
    if(antDP>antDNP):
        local_npc = local_npc + 1
